...
public class DiWeightedGraph<V, E> implements Graph<V, E> {
    ...
    /**
     * @param v vertice 
     * @return arestas incidentes ao vertice v em parametro
    ...
    public Iterable<Edge<E, V>> incidentEdges(Vertex<V> v) throws InvalidEdgeException {
        ...
        for (Edge<E, V> edge : edges.values()) {
            ...
            if( ((MyEdge)edge).vertexInbound == v) { /* edge.vertices()[0] == v || edge.vertices()[1] == v */
    ...
    /**
     * @param v vertice 
     * @return arestas acedentes ao vertice v em parametro
     * @throws InvalidEdgeException 
     */
    @Override
    public Iterable<Edge<E, V>> acedenteEdges(Vertex<V> v) throws InvalidEdgeException {
        checkVertex(v);
        
        List<Edge<E,V>> acedenteEdges = new ArrayList<>();
        for (Edge<E, V> edge : edges.values()) {
            
            if( ((MyEdge)edge).vertexOutbound == v ) { 
                acedenteEdges.add(edge);
            }
            
        }
        return acedenteEdges;
    }
